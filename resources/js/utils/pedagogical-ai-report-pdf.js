                ...
                [
                    {
                        stack: [
                            ...
                            item.recommendation
                                ...
                                : null,
                            {
                                text: `Ejemplo específico: ${value(
                                    item.improvement_example,
                                    "No disponible en esta versión del informe."
                                )}`,
                                alignment: "justify",
                                color: colors.amber,
...
function listSection(title, items, accent) {
    const rows =
        Array.isArray(items) && items.length
            ...
            : [
                  {
                      text: "Sin antecedentes registrados.",
                      color: colors.muted,
                      italics: true,
                  },
              ];
...
function detailedFinding(item, miscellaneous = false) {
    ...
    const description = value(
        miscellaneous ? item.finding : item.description,
        "Sin descripción."
    );
    const severityColor =
...
export function buildPedagogicalAiReportPdfDefinition(instrument, aiReport) {
    ...
    const coveredWidth = Math.max(
        1,
        Math.round((width * statistics.evidence_coverage_percentage) / 100)
    );
    const remainingWidth = Math.max(1, width - coveredWidth);
    const course = value(
        instrument?.courses?.map((item) => item.name).join(", ") ||
            instrument?.grade_label
    );
    const file = value(
    ...
    const ownerLabel =
        instrument?.workflow_status === "draft" ? "RESPONSABLE" : "DOCENTE";
        ...
        footer: (currentPage, pageCount) => ({
            ...
            stack: [
                ...
                },
                {
                    columns: [
                        {
                            text: "GESTIÓN PEDAGÓGICA  /  REVISIÓN DOCUMENTAL",
                            color: colors.muted,
                            fontSize: 6.1,
                            bold: true,
                            characterSpacing: 0.25,
                        ...
                        {
                            text: `${String(currentPage).padStart(
                                2,
                                "0"
        ...
        content: [
                        ...
                        [
                            {
                                stack: [
                                    ...
                                    {
                                        text: "Informe de retroalimentación documental",
                                        color: colors.white,
                                        bold: true,
                                        fontSize: 19,
                                        lineHeight: 1.08,
                                    ...
                                    {
                                        text: value(instrument?.school?.name),
                                        color: "#D9E8EF",
                                        fontSize: 8.2,
                                    },
                                ],
                            ...
                            },
                            {
                                stack: [
                                    ...
                                    {
                                        text: `VERSIÓN ${value(
                                            aiReport?.file_version ||
                                                instrument?.latest_file
                                                    ?.version,
                                            "1"
                                        )}`,
                                        alignment: "center",
            ...
            },
            {
                table: {
                    ...
                    body: [
                        [
                            { text: ownerLabel, style: "metaLabel" },
                            {
                                text: value(instrument?.owner?.name),
                                style: "metaValue",
                            },
                            { text: "ASIGNATURA", style: "metaLabel" },
                            {
                                text: value(instrument?.subject?.name),
                                style: "metaValue",
                            },
                        ],
                        [
                            ...
                            { text: "VERSIÓN", style: "metaLabel" },
                            {
                                text: `v${value(
                                    aiReport?.file_version ||
                                        instrument?.latest_file?.version,
                                    "1"
                                )} · ${formatDateTime(aiReport?.finished_at)}`,
                                style: "metaValue",
                            },
                        ],
                        [
            ...
            sectionHeader("Estadística de resumen"),
            {
                columns: [
                    ...
                    metric(
                        "Parciales",
                        statistics.partially_meets,
                        colors.amberSoft,
                        colors.amber
                    ),
                    metric(
                        "No se ajustan",
                        statistics.does_not_meet,
                        colors.redSoft,
                        colors.red
                    ),
                    metric(
                        "No evidenciados",
                        statistics.not_evidenced,
                        colors.soft,
                        colors.muted
                    ),
                    metric(
                        "Misceláneos",
                        statistics.miscellaneous_findings,
                        colors.tealSoft,
                        colors.teal
                    ),
                ],
            ...
            },
            {
                columns: [
                    ...
                    {
                        text: `${statistics.evidence_coverage_percentage}%`,
                        color: colors.teal,
                        bold: true,
                        alignment: "right",
                        fontSize: 7,
            ...
            {
                table: {
                    widths: [coveredWidth, remainingWidth],
                    body: [
                        [
                            {
                                text: "",
                ...
                const dimensionHeader = {
                    table: {
                        ...
                        body: [
                            [
                                {
                                    text: group.dimension.toUpperCase(),
                                    color: colors.violet,
                                    fillColor: colors.violetSoft,
                                    bold: true,
                                    fontSize: 7.2,
            ...
            {
                columns: [
                    listSection("Fortalezas", report.strengths, colors.green),
                    listSection(
                        "Recomendaciones",
            ...
            ...(() => {
                if (!report.observations?.length) {
                    return [
                        ...
                        {
                            text: "No se registraron observaciones adicionales.",
                            color: colors.muted,
                            italics: true,
                            margin: [0, 5, 0, 12],
                        },
                ...
                const [firstObservation, ...remainingObservations] =
                    report.observations;
                return [
                    {
                        unbreakable: true,
                        stack: [
                            sectionHeader("Observaciones detectadas"),
                            detailedFinding(firstObservation),
                        ],
                    },
                    ...remainingObservations.map((item) =>
                        detailedFinding(item)
                    ),
                ];
            })(),
            ...(() => {
                const header = sectionHeader(
                    ...
                    "Control complementario de puntajes, totales, ponderaciones y coherencia interna",
                    colors.navy
                );
                if (!report.miscellaneous_findings?.length) {
                    return [
                        header,
                        {
                            text: "No se detectaron incoherencias adicionales verificables.",
                            color: colors.green,
                ...
                const [firstFinding, ...remainingFindings] =
                    report.miscellaneous_findings;
                return [
                    {
                        unbreakable: true,
                        stack: [header, detailedFinding(firstFinding, true)],
                    },
                    ...remainingFindings.map((item) =>
                        detailedFinding(item, true)
                    ),
                ];
            })(),
            {
                ...
                stack: [
                    sectionHeader(
                        "Mensaje sugerido para el docente",
                        null,
                        colors.violet
                    ),
                    {
                        table: {
                            ...
                            body: [
                                [
                                    ...
                                    {
                                        text: value(
                                            report.suggested_teacher_message,
...
export function pedagogicalAiReportFilename(
    instrument,
    generatedAt = new Date()
) {
    return `informe-retroalimentacion-${safeFilenamePart(
        instrument?.title
    )}-${localDateStamp(generatedAt)}.pdf`;
}

export async function downloadPedagogicalAiReportPdf(
    instrument,
    aiReport,
    generatedAt = new Date()
) {

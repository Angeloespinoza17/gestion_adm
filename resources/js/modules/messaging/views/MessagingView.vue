...
const messages = computed(
    () => store.state.messagesByConversation[activeId.value] || []
);
const participants = computed(
    () =>
        activeConversation.value?.participants?.filter(
            (item) => !item.left_at
        ) || []
);
...
const canManageGroup = computed(
    () =>
        ...
        ["owner", "admin"].includes(activeParticipant.value?.role)
);
...
const canSend = computed(() => {
    if (!activeConversation.value || activeConversation.value.is_locked)
        return false;
...
const attachmentAccept = computed(() =>
    (store.state.config.attachments?.extensions || [])
        .map((item) => `.${item}`)
        .join(",")
);
const groupReady = computed(
    () =>
        groupTitle.value.trim().length >= 2 &&
...
const connectionLabel = computed(
    () =>
        ...
        }[store.state.pollingConnectionState] || "Actualización automática")
);
const acknowledgementComplete = (message) => {
    const summary = message.acknowledgement_summary;
    return (
        Number(summary?.total || 0) > 0 &&
        Number(summary.acknowledged || 0) >= Number(summary.total || 0)
    );
};
const acknowledgementNames = (message) =>
    (message.acknowledgement_summary?.acknowledged_by || [])
        .map((item) => item.name)
        .filter(Boolean)
        .join(", ");
...
function handleTimelineScroll() {
    if (
        timeline.value?.scrollTop <= 100 &&
        store.state.hasOlderByConversation[activeId.value]
    ) {
    ...
    async () => {
        ...
        const followsLatest =
            ...
            element.scrollHeight - element.scrollTop - element.clientHeight <
                180;
        await nextTick();
        if (followsLatest && timeline.value)
            timeline.value.scrollTop = timeline.value.scrollHeight;
    }
);
...
function conversationParams() {
    ...
    if (["group", "announcement"].includes(activeFilter.value))
        params.type = activeFilter.value;
    return params;
}

async function loadConversations() {
    await store.loadConversations(conversationParams());
}
...
async function createGroup() {
    ...
    try {
        ...
        createError.value =
            error.response?.data?.message || "No fue posible crear el grupo.";
    } finally {
        createSaving.value = false;
    }
}
...
function openGroupManager() {
    ...
    groupEditForm.value = {
        ...
        only_admins_can_write: Boolean(
            activeConversation.value.only_admins_can_write
        ),
...
async function saveGroupSettings() {
    ...
    try {
        ...
        if (
            groupEditForm.value.is_locked !==
            Boolean(activeConversation.value.is_locked)
        ) {
...
async function addGroupParticipant(user) {
    ...
    try {
        ...
        groupEditError.value =
            error.response?.data?.message ||
            "No fue posible agregar al integrante.";
    } finally {
...
async function toggleGroupAdmin(participant) {
    ...
    try {
        ...
        await refreshManagedGroup(
            role === "admin"
                ? `${participant.name} ahora es administrador.`
                : `${participant.name} ahora es integrante.`
        );
    } catch (error) {
        groupEditError.value =
            error.response?.data?.message || "No fue posible cambiar el rol.";
    } finally {
        groupActionId.value = null;
    }
}
...
async function removeGroupParticipant(participant) {
    if (!window.confirm(`¿Retirar a ${participant.name} de este grupo?`))
        return;
    ...
    try {
        ...
        await refreshManagedGroup(
            `${participant.name} fue retirado del grupo.`
        );
    } catch (error) {
        groupEditError.value =
            error.response?.data?.message ||
...
async function transferGroupOwnership(participant) {
    if (
        !window.confirm(
            `¿Transferir la propiedad del grupo a ${participant.name}?`
        )
    )
        return;
    ...
    try {
        ...
        await refreshManagedGroup(
            `${participant.name} ahora es propietario del grupo.`
        );
    } catch (error) {
        groupEditError.value =
            error.response?.data?.message ||
            "No fue posible transferir la propiedad.";
    } finally {
        groupActionId.value = null;
    }
}
...
async function removeUpload(upload) {
    ...
    try {
        await api.removeUpload(upload.token);
    } catch (_) {
...
async function acknowledge() {
    if (
        ackMessage.value.acknowledgement_comment_required &&
        !ackComment.value.trim()
    )
        return;
    ...
    <Layout>
        <div class="messaging-page">
            <header class="messaging-hero">
                ...
                <div class="hero-actions">
                    <div
                        ...
                        :class="store.state.pollingConnectionState"
                    >
            ...
            <section class="messenger-shell">
                ...
                >
                    <div class="panel-overview">
                        <div>
                            ...
                            <strong
                                >{{
                                    store.state.summary.unread_messages || 0
                                }}
                                sin leer</strong
                            >
                        </div>
                    ...
                    <div class="conversation-list">
                        <div
                            v-if="store.state.loading.conversations"
                            class="conversation-skeletons"
                        ...
                        <button
                            ...
                            :class="{
                                selected: conversation.public_id === activeId,
                        ...
                        >
                            <span
                                ...
                                :class="avatarTone(conversation.title)"
                            >
                                <img
                                    v-if="conversationPhoto(conversation)"
                                    :src="conversationPhoto(conversation)"
                                    :alt="conversation.title"
                                />
                                <i
                                    v-else-if="conversation.type !== 'direct'"
                                    class="bx"
                                    :class="typeMeta(conversation.type).icon"
                                ></i>
                                <span v-else>{{
                            ...
                            <span class="conversation-content">
                                ...
                                <span
                                    class="conversation-line conversation-line--preview"
                                >
                                    <small>{{
                                        messagePreview(conversation)
                                    }}</small>
                                    <b v-if="conversation.unread_count">{{
                                ...
                                >
                                    <i
                                        class="bx"
                                        :class="
                                            typeMeta(conversation.type).icon
                                        "
                                    ></i
                                    >{{ typeMeta(conversation.type).label }}
                                </span>
                            </span>
                        </button>
                        ...
                        <div
                            v-if="
                                !store.state.loading.conversations &&
                            ...
                            <p>
                                {{
                                    query
                                        ...
                                        : "Inicia una conversación directa o crea un grupo para tu equipo."
                                }}
                            </p>
                            <button type="button" @click="openCreate('direct')">
                                <i class="bx bx-plus"></i> Nueva conversación
                            </button>
                        </div>
                    </div>
                </aside>
                ...
                >
                    <template v-if="activeConversation">
                        <header class="thread-header">
                            ...
                            <span
                                class="conversation-avatar conversation-avatar--header"
                                :class="avatarTone(activeConversation.title)"
                            >
                                <img
                                    v-if="conversationPhoto(activeConversation)"
                                    :src="conversationPhoto(activeConversation)"
                                ...
                                <i
                                    ...
                                    class="bx"
                                    :class="
                                        typeMeta(activeConversation.type).icon
                                    "
                                ></i>
                                <span v-else>{{
                            ...
                            <div class="thread-identity">
                                ...
                                <span
                                    ><i
                                        class="bx"
                                        :class="
                                            typeMeta(activeConversation.type)
                                                .icon
                                        "
                                    ></i
                                    >{{ participantCaption }}</span
                                >
                            </div>
                            <div class="thread-header-actions">
                                <span
                                    v-if="activeConversation.is_locked"
                                    class="locked-pill"
                        ...
                        >
                            <div class="timeline-intro">
                                <span
                                    class="conversation-avatar conversation-avatar--intro"
                                    :class="
                                        avatarTone(activeConversation.title)
                                    "
                                >
                                    <i
                                        v-if="
                                    ...
                                    {{
                                        ...
                                        `Este es el comienzo de ${
                                            activeConversation.type === "direct"
                                                ...
                                                : "este espacio de colaboración"
                                        }.`
                                    }}
                                </p>
                                <span class="privacy-note"
                                    ><i class="bx bx-lock-alt"></i> Solo los
                                    participantes pueden ver este
                                    contenido</span
                                >
                            </div>
                            ...
                            <button
                                v-if="
                                    store.state.hasOlderByConversation[activeId]
                                "
                                class="load-older"
                                type="button"
                                :disabled="store.state.loading.older"
                                @click="loadOlderPreservingPosition"
                            >
                                <i
                                    class="bx"
                                    :class="
                                        store.state.loading.older
                            ...
                            >
                                <div
                                    v-if="
                                        !index ||
                                        dateKey(messages[index - 1].sent_at) !==
                                            dateKey(message.sent_at)
                                    "
                                    class="date-divider"
                                >
                                    <span>{{
                                ...
                                >
                                    <span
                                        ...
                                        :class="
                                            avatarTone(message.sender?.name)
                                        "
                                    >
                                        <img
                                            v-if="message.sender?.photo"
                                    ...
                                    <div class="message-stack">
                                        ...
                                        <div class="message-bubble">
                                            <div
                                                ...
                                                class="formal-heading"
                                            >
                                                <span
                                                    ><i
                                                        class="bx bx-file-blank"
                                                    ></i>
                                                    Comunicación formal</span
                                                >
                                                <b
                                                    :class="`priority-tag priority-tag--${message.priority}`"
                                                    >{{ message.priority }}</b
                                                >
                                            </div>
                        ...
                        <form class="composer" @submit.prevent="send">
                            <div v-if="replyingTo" class="composer-reply">
                                ...
                                <span
                                    ><strong
                                        >Respondiendo a
                                        {{ replyingTo.sender?.name }}</strong
                                    ><small>{{ replyingTo.body }}</small></span
                                >
                                <button
                                    type="button"
                            ...
                            <div v-if="uploads.length" class="composer-uploads">
                                ...
                                >
                                    ...
                                    <span
                                        ...
                                        ><small>Listo para enviar</small></span
                                    >
                                    <button
                                        type="button"
                            ...
                            <div
                                v-if="formal || requiresAck"
                                class="formal-settings"
                            >
                                ...
                                <template v-if="requiresAck">
                                    ...
                                    <label class="mini-check"
                                        ><input
                                            ...
                                            type="checkbox"
                                        />
                                        Permitir respuestas</label
                                    >
                                </template>
                            </div>

                            <div
                                v-if="uploadError || sendError"
                            ...
                            <div v-if="!canSend" class="composer-locked">
                                ...
                                <span>{{
                                    activeConversation.is_locked
                                        ? "Esta conversación está bloqueada temporalmente."
                            ...
                            <div v-else class="composer-box">
                                <textarea
                                    ...
                                    rows="1"
                                    :maxlength="
                                        store.state.config.messages?.max_length
                                    "
                                ...
                                <div class="composer-toolbar">
                                    <div class="composer-tools">
                                        ...
                                        <button
                                            type="button"
                                            :class="{
                                                active: formal || requiresAck,
                                            }"
                                            title="Opciones formales"
                                            @click="formal = !formal"
                                        >
                                            <i class="bx bx-file-blank"></i>
                                        </button>
                    ...
                    <div v-else class="thread-welcome">
                        ...
                        <p>
                            Selecciona una conversación o crea un grupo para
                            mantener a tu equipo conectado.
                        </p>
                        <div class="welcome-actions">
                            <button type="button" @click="openCreate('direct')">
                                <i class="bx bx-message-rounded-add"></i> Nuevo
                                mensaje
                            </button>
                            <button type="button" @click="openCreate('group')">
                ...
                <aside v-if="activeConversation" class="details-panel">
                    <div class="details-hero">
                        <span
                            class="conversation-avatar conversation-avatar--details"
                            :class="avatarTone(activeConversation.title)"
                        >
                            <img
                                v-if="conversationPhoto(activeConversation)"
                            ...
                            <i
                                ...
                                class="bx"
                                :class="typeMeta(activeConversation.type).icon"
                            ></i>
                            <span v-else>{{
                                initials(activeConversation.title)
                            }}</span>
                        ...
                        <span class="details-type"
                            ><i
                                ...
                                :class="typeMeta(activeConversation.type).icon"
                            ></i
                            >{{ typeMeta(activeConversation.type).label }}</span
                        >
                        <p>
                            {{
                                activeConversation.description ||
                                "Conversación institucional segura."
                            }}
                        </p>
                        <button
                            v-if="canManageGroup"
                            class="details-manage-button"
                            type="button"
                            @click="openGroupManager"
                        >
                            <i class="bx bx-cog"></i> Administrar grupo
                        </button>
                    </div>
                    ...
                    <div class="details-stats">
                        ...
                        <div>
                            ...
                            ><strong>{{
                                store.state.summary.pending_acknowledgements ||
                                0
                            }}</strong
                            ><small>Acuses</small>
                        </div>
                    ...
                    <section class="participant-section">
                        ...
                        <div class="participant-list">
                            <div
                                v-for="participant in participants.slice(0, 10)"
                                :key="participant.id"
                                class="participant-row"
                            >
                                <span
                                    class="participant-avatar"
                                    :class="avatarTone(participant.name)"
                                >
                                    <img
                    ...
                    <div class="details-security">
                        ...
                        ><span
                            ...
                            ><small
                                >Acceso limitado a participantes.</small
                            ></span
                        >
                    </div>
                </aside>
                ...
                >
                    ...
                    <div class="create-mode-switch">
                        ...
                        >
                            ...
                            ><span
                                ...
                                ><small>Conversación uno a uno</small></span
                            >
                        </button>
                        <button
                            type="button"
                        ...
                        >
                            ...
                            ><span
                                ...
                                ><small>Colaboración en equipo</small></span
                            >
                        </button>
                    </div>
                    ...
                    >
                        ...
                        <label class="admin-setting">
                            ...
                            <span
                                ><strong
                                    ...
                                    escribir</strong
                                ><small
                                    >Los integrantes podrán leer, pero no enviar
                                    mensajes.</small
                                ></span
                            >
                            <input
                                v-model="groupOnlyAdmins"
                                type="checkbox"
                                class="switch-input"
                            />
                        </label>
                        ...
                        <div v-if="createError" class="dialog-error">
                            <i class="bx bx-error-circle"></i>{{ createError }}
                        </div>
                        <footer class="create-footer">
                            <span
                                ><i class="bx bx-info-circle"></i> Serás
                                propietario y podrás administrar
                                integrantes.</span
                            >
                            <div>
                                <button
                    ...
                    <div v-else class="direct-picker">
                        <label
                            class="user-search-field user-search-field--large"
                        >
                            <i class="bx bx-search"></i>
                            <input
                                v-model="userQuery"
            ...
            <div
                v-if="groupEditOpen"
                class="modal-layer"
                @click.self="closeGroupManager"
            >
                <section
                    class="create-dialog group-manage-dialog"
                    role="dialog"
                    aria-modal="true"
                ...
                >
                    ...
                    <header class="create-heading group-manage-heading">
                        ...
                        <div>
                            <span class="hero-kicker"
                                >Administración del grupo</span
                            >
                            <h2 id="group-manage-title">
                                Configura {{ activeConversation.title }}
                            </h2>
                            <p>
                                Actualiza la identidad, permisos e integrantes
                                del espacio.
                            </p>
                        </div>
                    </header>
                    ...
                    <div class="group-manage-grid">
                        <form
                            ...
                            @submit.prevent="saveGroupSettings"
                        >
                            <header>
                                <span><i class="bx bx-slider-alt"></i></span>
                                <div>
                                    <strong>Información y permisos</strong
                                    ><small
                                        >Define cómo funciona el grupo.</small
                                    >
                                </div>
                            </header>
                            ...
                            <label class="manage-toggle-row">
                                ...
                                <span
                                    ><strong
                                        >Solo administradores escriben</strong
                                    ><small
                                        >Los integrantes conservarán acceso de
                                        lectura.</small
                                    ></span
                                >
                                <input
                                    v-model="
                                        groupEditForm.only_admins_can_write
                                    "
                                    type="checkbox"
                                    class="switch-input"
                                />
                            ...
                            <label
                                class="manage-toggle-row manage-toggle-row--warning"
                            >
                                <span><i class="bx bx-lock-alt"></i></span>
                                <span
                                    ><strong>Bloquear temporalmente</strong
                                    ><small
                                        >Nadie podrá enviar mensajes hasta
                                        desbloquearlo.</small
                                    ></span
                                >
                                <input
                                    v-model="groupEditForm.is_locked"
                                    type="checkbox"
                                    class="switch-input"
                                />
                            </label>
                        ...
                        <section class="group-members-card">
                            ...
                            <label
                                class="user-search-field group-member-search"
                            >
                                <i class="bx bx-search"></i>
                                <input
                            ...
                            <div
                                v-if="manageUserQuery.trim().length >= 2"
                                class="manage-candidates"
                            >
                                <button
                            ...
                            <div class="managed-participant-list">
                                ...
                                >
                                    ...
                                    <span class="managed-participant-copy"
                                        ><strong>{{
                                            participant.id === currentUser.id
                                                ? `${participant.name} (Tú)`
                                                : participant.name
                                    ...
                                    <i
                                        v-else-if="participant.role === 'owner'"
                                        class="owner-crown bx bx-crown"
                                        title="Propietario"
                                    ></i>
                                </article>
                            </div>
                    ...
                    <div v-if="groupEditError" class="dialog-error">
                        <i class="bx bx-error-circle"></i>{{ groupEditError }}
                    </div>
                    <div v-if="groupEditNotice" class="dialog-success">
                        <i class="bx bx-check-circle"></i>{{ groupEditNotice }}
                    </div>
                    <footer class="group-manage-footer">
                        <span
                            ><i class="bx bx-info-circle"></i> Los cambios se
                            aplican inmediatamente.</span
                        ><button
                            type="button"
                            class="cancel-button"
                            @click="closeGroupManager"
            ...
            >
                <section
                    ...
                    aria-modal="true"
                >
                    <span class="ack-dialog-icon"
                        ><i class="bx bx-check-shield"></i
                    ></span>
                    <span class="hero-kicker">Confirmación explícita</span>
                    <h2>
                        ¿Confirmas que recibiste y revisaste esta comunicación?
                    </h2>
                    <p>
                        Esta acción registra fecha, versión y hash del
                        contenido. No representa aceptación ni firma
                        electrónica.
                    </p>
                    <label v-if="ackMessage.acknowledgement_comment_required"
                    ...
                    <div class="ack-dialog-actions">
                        <button type="button" @click="ackMessage = null">
                            Cancelar</button
                        ><button
                            type="button"
                            :disabled="
                                ackSaving ||
                                (ackMessage.acknowledgement_comment_required &&
                                    !ackComment.trim())
                            "
                            @click="acknowledge"
                        >
...
.connection-state {
    ...
    font-size: 11px;
    font-weight: 650;
    box-shadow: 0 3px 14px rgba(25, 35, 60, 0.04);
}
.connection-dot {
    width: 7px;
    height: 7px;
...
.connection-state.connected .connection-dot {
    background: #25b47e;
    box-shadow: 0 0 0 4px rgba(37, 180, 126, 0.12);
}
.create-group-button,
.new-message-button {
    height: 42px;
...
.new-message-button {
    color: #fff;
    background: linear-gradient(135deg, #6664ee, #4c49d7);
    border: 0;
    box-shadow: 0 9px 22px rgba(86, 84, 232, 0.24);
}
...
.new-message-button:hover {
    box-shadow: 0 12px 28px rgba(86, 84, 232, 0.32);
}
...
.messenger-shell {
    display: grid;
    grid-template-columns: 350px minmax(440px, 1fr) 310px;
    height: calc(100vh - 184px);
    min-height: 620px;
    overflow: hidden;
    background: #fff;
    border: 1px solid rgba(222, 226, 237, 0.9);
    border-radius: 22px;
    box-shadow: 0 20px 55px rgba(25, 35, 60, 0.09);
}
.conversation-panel,
.thread-panel,
.details-panel {
...
.user-search-field:focus-within {
    background: #fff;
    border-color: #c9cbfb;
    box-shadow: 0 0 0 3px rgba(86, 84, 232, 0.08);
}
...
.conversation-list {
    ...
    overflow-y: auto;
    padding: 8px;
    scrollbar-color: #d7dbe5 transparent;
...
.conversation-card.selected {
    background: linear-gradient(135deg, #f0f1ff 0%, #f8f7ff 100%);
    border-color: #e0e0fc;
}
...
.tone-indigo {
    background: linear-gradient(145deg, #6a67ec, #4947c9);
}
.tone-violet {
    background: linear-gradient(145deg, #9b6de3, #7748c0);
}
.tone-cyan {
    background: linear-gradient(145deg, #38a9d8, #247cae);
}
.tone-emerald {
    background: linear-gradient(145deg, #39b98a, #238862);
}
.tone-coral {
    background: linear-gradient(145deg, #ef7e73, #cf574e);
}
.tone-blue {
    background: linear-gradient(145deg, #5489e8, #3567bd);
}
.presence-dot {
    position: absolute;
    right: 1px;
...
.conversation-skeletons span {
    display: block;
    height: 72px;
    margin: 4px;
    background: linear-gradient(90deg, #f3f4f7 25%, #fafafd 45%, #f3f4f7 65%);
...
.message-timeline {
    ...
    overflow-x: hidden;
    overflow-y: auto;
    padding: 0 clamp(16px, 3vw, 38px) 28px;
    background-color: #f7f8fb;
    background-image: radial-gradient(
            circle at 12% 18%,
            rgba(86, 84, 232, 0.035) 0,
            transparent 24%
        ),
        radial-gradient(
            circle at 88% 64%,
            rgba(37, 180, 170, 0.035) 0,
            transparent 20%
        );
    scrollbar-color: #d2d7e2 transparent;
    scrollbar-width: thin;
}
.timeline-intro {
    max-width: 470px;
...
.conversation-avatar--intro {
    width: 58px;
    height: 58px;
    margin: 0 auto 12px;
    border-radius: 18px;
    font-size: 18px;
    box-shadow: 0 10px 25px rgba(42, 50, 90, 0.14);
}
.conversation-avatar--intro i {
    font-size: 26px;
}
.timeline-intro h3 {
    margin: 0;
...
.message-bubble {
    ...
    box-shadow: 0 4px 14px rgba(27, 39, 67, 0.045);
}
.message-item.own .message-bubble {
    color: #fff;
    background: linear-gradient(145deg, #6462e9, #504dd5);
    border-color: transparent;
    border-radius: 16px 6px 16px 16px;
    box-shadow: 0 7px 20px rgba(73, 70, 198, 0.16);
}
.message-bubble p {
    margin: 0;
...
.message-item.formal .message-stack {
    max-width: min(84%, 720px);
}
.message-item.formal .message-bubble {
    padding: 14px;
    color: var(--msg-ink);
    background: #fff;
    border-top: 3px solid var(--msg-brand);
...
.reply-preview {
    ...
    color: #657086;
    background: #f3f4f8;
    border-left: 3px solid #9795ef;
    border-radius: 7px;
}
.message-item.own:not(.formal) .reply-preview {
...
.message-item.own:not(.formal) .message-attachments a {
    color: #fff;
    background: rgba(255, 255, 255, 0.12);
    border-color: rgba(255, 255, 255, 0.14);
}
.message-attachments a > span:first-child {
    width: 32px;
    height: 32px;
    display: grid;
    place-items: center;
...
.message-attachments a > span:nth-child(2) {
    flex: 1;
    min-width: 0;
    display: flex;
    flex-direction: column;
...
.acknowledgement-card small {
    font-size: 8.5px;
}
.acknowledgement-card button {
    padding: 5px 8px;
    color: #fff;
    background: #936914;
    border: 0;
...
.acknowledgement-card.acknowledged {
    color: #277954;
    background: #eaf8f2;
    border-color: #c5e9d9;
}
...
.message-item.own .reaction-list {
    justify-content: flex-end;
}
.reaction-list button {
    ...
    border-radius: 12px;
    font-size: 10px;
    box-shadow: 0 2px 6px rgba(25, 35, 60, 0.04);
}
...
.message-actions {
    ...
    border: 1px solid var(--msg-line);
    border-radius: 9px;
    box-shadow: 0 7px 18px rgba(25, 35, 60, 0.09);
...
.composer-box {
    ...
    border-radius: 15px;
    box-shadow: 0 5px 18px rgba(28, 39, 65, 0.055);
    transition: border 0.18s ease, box-shadow 0.18s ease;
}
.composer-box:focus-within {
    border-color: #bfc0f5;
    box-shadow: 0 5px 18px rgba(65, 63, 183, 0.09),
        0 0 0 3px rgba(86, 84, 232, 0.06);
}
.composer textarea {
    width: 100%;
    min-height: 43px;
...
.send-tools button {
    ...
    place-items: center;
    color: #fff;
    background: linear-gradient(135deg, #6664ee, #4d4ad3);
    border: 0;
    border-radius: 10px;
    font-size: 17px;
    box-shadow: 0 6px 14px rgba(86, 84, 232, 0.24);
}
.send-tools button:disabled {
    opacity: 0.45;
    box-shadow: none;
}
.composer-reply {
...
.composer-uploads > span > i {
    color: var(--msg-brand);
    font-size: 19px;
}
.composer-uploads > span > span {
    flex: 1;
    min-width: 0;
    display: flex;
    flex-direction: column;
}
.composer-uploads strong {
    overflow: hidden;
    font-size: 9px;
    text-overflow: ellipsis;
    white-space: nowrap;
}
.composer-uploads small {
    color: #38a378;
    font-size: 8px;
}
.formal-settings {
...
.thread-welcome {
    ...
    padding: 30px;
    text-align: center;
    background: radial-gradient(circle at 50% 44%, #fff 0, #f7f8fb 55%);
}
.welcome-visual {
    position: relative;
    width: 116px;
    ...
    margin-bottom: 20px;
    color: var(--msg-brand);
    background: linear-gradient(145deg, #fff, #eeefff);
    border: 1px solid #e1e2fa;
    border-radius: 35px;
    box-shadow: 0 22px 50px rgba(68, 65, 180, 0.13);
...
.orb {
    ...
    border-radius: 50%;
    box-shadow: 0 5px 14px rgba(44, 161, 134, 0.25);
}
.orb--one {
    top: -7px;
    right: 4px;
...
.conversation-avatar--details {
    width: 68px;
    height: 68px;
    margin: 0 auto 12px;
    border-radius: 21px;
    font-size: 20px;
    box-shadow: 0 11px 28px rgba(36, 47, 76, 0.15);
}
.conversation-avatar--details i {
    font-size: 29px;
}
.details-hero h3 {
    margin: 0 0 6px;
...
.participant-section header > span {
    min-width: 20px;
    height: 20px;
    display: grid;
    place-items: center;
...
.ack-dialog {
    ...
    border-radius: 22px;
    box-shadow: 0 30px 90px rgba(9, 16, 34, 0.3);
}
.dialog-close {
    position: absolute;
    z-index: 2;
...
.create-icon {
    ...
    place-items: center;
    color: #fff;
    background: linear-gradient(145deg, #6866ec, #4c49ce);
    border-radius: 15px;
    font-size: 23px;
    box-shadow: 0 8px 20px rgba(86, 84, 232, 0.22);
}
.create-heading h2 {
    margin: 3px 0 3px;
    font-size: 21px;
...
.create-mode-switch button > i {
    font-size: 20px;
}
.create-mode-switch button > span {
    display: flex;
    flex-direction: column;
}
.create-mode-switch strong {
    font-size: 10px;
}
...
.create-mode-switch button.active {
    ...
    background: #fff;
    border-color: #e5e6ed;
    box-shadow: 0 3px 10px rgba(25, 35, 60, 0.06);
}
.field-grid {
    display: grid;
    grid-template-columns: 1fr 1fr;
...
.selected-members .participant-avatar {
    width: 24px;
    height: 24px;
    border-radius: 7px;
    font-size: 7px;
...
.people-results > button > span:nth-child(2) {
    flex: 1;
    min-width: 0;
    display: flex;
    flex-direction: column;
}
.people-results strong,
.people-results small {
    overflow: hidden;
    text-overflow: ellipsis;
    white-space: nowrap;
}
...
.people-results > button > i {
    color: #9ba3b3;
    font-size: 17px;
}
.people-results > button.selected > i {
    color: var(--msg-brand);
}
.people-results > p {
    grid-column: 1 / -1;
...
.people-results--direct {
    max-height: 320px;
    grid-template-columns: 1fr;
    margin-top: 10px;
}
.people-results--direct > button {
    padding: 9px;
}
.people-results--direct .participant-avatar {
    width: 39px;
    height: 39px;
}
.search-prompt {
...
.admin-setting > span:nth-child(2) {
    flex: 1;
    display: flex;
    flex-direction: column;
}
...
.confirm-button {
    ...
    background: var(--msg-brand);
    border: 0;
    box-shadow: 0 6px 15px rgba(86, 84, 232, 0.2);
}
...
.ack-dialog-actions button:last-child {
    color: #fff;
    background: var(--msg-brand);
...
.group-settings-card > header > span,
.group-members-card > header > span {
    width: 35px;
    height: 35px;
    display: grid;
    place-items: center;
...
.group-settings-card > header > div,
.group-members-card > header > div {
    display: flex;
    flex-direction: column;
}
.group-settings-card > header strong,
.group-members-card > header strong {
    font-size: 10.5px;
}
.group-settings-card > header small,
.group-members-card > header small {
    color: var(--msg-muted);
    font-size: 8px;
}
.manage-field {
...
.manage-toggle-row > span:first-child {
    width: 31px;
    height: 31px;
    display: grid;
    place-items: center;
...
.manage-toggle-row > span:nth-child(2) {
    flex: 1;
    display: flex;
    flex-direction: column;
}
.manage-toggle-row strong {
    font-size: 8.5px;
}
.manage-toggle-row small {
    color: var(--msg-muted);
    font-size: 7.5px;
    line-height: 1.35;
}
.manage-toggle-row--warning > span:first-child {
    color: #a46e1d;
    background: #fff4df;
}
.save-group-button {
    ...
    margin-top: 12px;
    color: #fff;
    background: linear-gradient(135deg, #6563ea, #504dd4);
    border: 0;
    border-radius: 10px;
    font-size: 9px;
    font-weight: 800;
    box-shadow: 0 7px 16px rgba(86, 84, 232, 0.18);
}
...
.manage-candidates button:last-of-type {
    border-bottom: 0;
}
.manage-candidates button:hover {
    background: #f4f4fc;
}
.manage-candidates button > span:nth-child(2) {
    flex: 1;
    min-width: 0;
    display: flex;
    flex-direction: column;
}
.manage-candidates strong,
.manage-candidates small {
    overflow: hidden;
    text-overflow: ellipsis;
    white-space: nowrap;
}
...
.managed-participant-list article {
    display: flex;
    align-items: center;
    gap: 8px;
    padding: 7px 4px;
...
.managed-participant-copy strong {
    overflow: hidden;
    font-size: 9px;
    text-overflow: ellipsis;
    white-space: nowrap;
}
.managed-participant-copy small {
    color: var(--msg-muted);
    font-size: 7.5px;
}
.participant-actions {
...
.participant-actions .participant-remove:hover {
    color: #bd4351;
    background: #fff0f2;
    border-color: #f0ccd1;
}

...
const form = reactive({ name: "", description: "", assessment_type: "summative", scheduled_on: "", weighting: null, maximum_score: null, grading_scale: "1_to_7", curriculum_objective_ids: [] });
...
const openCreate = () => { if (!props.canManage) return; editing.value = null; Object.assign(form, { name: "", description: "", assessment_type: "summative", scheduled_on: new Date().toISOString().slice(0, 10), weighting: null, maximum_score: null, grading_scale: "1_to_7", curriculum_objective_ids: [] }); showForm.value = true; };
const openEdit = (item) => { if (!props.canManage) return; editing.value = item; Object.assign(form, { name: item.name || item.title || "", description: item.description || "", assessment_type: item.assessment_type || item.type || "summative", scheduled_on: String(item.scheduled_on || item.date || "").slice(0, 10), weighting: item.weighting ?? null, maximum_score: item.maximum_score ?? null, grading_scale: item.grading_scale || "1_to_7", curriculum_objective_ids: item.curriculum_objective_ids || [] }); showForm.value = true; };
  ...
  <section class="lcd-assessments" aria-labelledby="lcd-assessments-title" :aria-busy="loading || saving">
    ...
    <BModal v-model="showForm" :title="editing ? 'Editar evaluación' : 'Nueva evaluación'" size="lg" hide-footer>
      <form class="row g-3" @submit.prevent="save">
        ...
        <div class="col-12 lcd-assessment-curriculum"><label class="form-label" for="lcd-assessment-curriculum">Objetivos curriculares oficiales (OA/OAT)</label><select id="lcd-assessment-curriculum" v-model="form.curriculum_objective_ids" class="form-select" multiple size="6" :disabled="!curriculumObjectives.length"><option v-for="objective in curriculumObjectives" :key="objective.id" :value="objective.id">{{ objective.code }} · {{ objective.description }}</option></select><small v-if="curriculumObjectives.length">Solo se pueden asociar objetivos del catálogo oficial versionado. Mantén Ctrl o ⌘ para seleccionar varios.</small><BAlert v-else-if="curriculumBlocker" show variant="warning" class="small mt-2 mb-0">{{ curriculumBlocker.message }} No se permite ingresar códigos OA manualmente.</BAlert><BAlert v-else-if="curriculumError" show variant="warning" class="small mt-2 mb-0">No se pudo consultar el catálogo curricular: {{ errorMessage(curriculumError) }}</BAlert><small v-else>No hay objetivos oficiales disponibles para este contexto.</small></div>
...
@media(max-width:760px){.lcd-assessments>header{align-items:flex-start;flex-direction:column}.lcd-assessments__summary{grid-template-columns:1fr}.lcd-results__footer{align-items:stretch;flex-direction:column}.lcd-results__footer>div{display:grid;grid-template-columns:1fr 1fr}}
</style>

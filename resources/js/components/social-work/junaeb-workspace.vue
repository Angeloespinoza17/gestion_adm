...
const eligiblePassStudents = computed(() => matrix.value.filter(student => {
  const level = `${student.current_enrollment?.course_section?.education_level?.name || ''} ${student.current_enrollment?.course_section?.display_name || ''}`.toLowerCase()
  return /(5|5°|quinto|6|6°|sexto|7|7°|séptimo|septimo|8|8°|octavo).*b[aá]sic|([1-4]|[1-4]°|primero|segundo|tercero|cuarto).*medi/.test(level)
}))
...
async function loadAll() {
  try {
    const [benefitResponse, deliveryResponse, passResponse, serviceResponse, deviceResponse, matrixResponse] = await Promise.all([
...
async function fetchAllMatrix() {
  const collected = []
  let page = 1
  let lastPage = 1
  do {
    const response = await api.get(`/support-matrix?per_page=100&page=${page}`)
...
function currentCourseId(studentId) { return props.students.find(item => Number(item.id) === Number(studentId))?.current_enrollment?.course_section_id || null }
...
async function saveBenefit() {
  try {
    ...
    success.value = 'Beneficio JUNAEB incorporado.'; close(); await loadAll()
  } catch {}
}
async function saveDelivery() {
  ...
  try {
    ...
    success.value = 'Entrega registrada. El acta PDF ya está disponible.'; close(); await loadAll()
  } catch {}
}
async function savePass() {
  try {
    ...
    success.value = 'Estado del pase escolar registrado.'; close(); await loadAll()
  } catch {}
}
async function markHasPass(student) {
  ...
  try {
    ...
    success.value = `Pase confirmado para ${name(student)}.`; await loadAll()
  } catch {}
}
async function saveService() {
  try {
    ...
    success.value = 'Servicio médico registrado.'; close(); await loadAll()
  } catch {}
}
async function saveDevice() {
  try {
    ...
    success.value = 'Ayuda o condición de apoyo registrada.'; close(); await loadAll()
  } catch {}
}

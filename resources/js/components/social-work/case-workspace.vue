...
const intervention = reactive({
  ...
  status: 'finalizada', confidentiality: 'restringido', participants: [], commitments: [],
})
...
async function addIntervention() {
  ...
  try {
    ...
    intervention.objective = ''; intervention.description = ''; intervention.result = ''; intervention.agreements = ''; intervention.next_action = ''; intervention.due_at = ''
      ...
      <div class="sw-modal-panel" :class="{ wide: modal === 'case', attention: modal === 'attention' }">
        ...
        <form v-else class="modal-body attention-form" @submit.prevent="addIntervention">
          ...
          <template v-else>
            ...
            <footer><button type="button" class="btn btn-light" @click="cancelAttention">Cancelar</button><button class="btn btn-primary" :disabled="api.loading || !intervention.objective.trim()"><i class="bx bx-save"></i> Guardar atención</button></footer>
          </template>
        </form>
      </div>
    </div>

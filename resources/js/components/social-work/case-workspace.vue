...
const intervention = reactive({
  id: null, record_updated_at: '', change_reason: '',
...
const canSubmitAttention = computed(() => intervention.objective.trim()
  ...
  && (!intervention.id || intervention.change_reason.trim().length >= 5)
...
function openInterventionEdit(item) {
  if (!item?.id || !activeCase.value || !canRegisterAttention.value) return
  const structured = item.structured_data || {}
  const participantTypes = structured.participant_types || [...new Set((item.participants || [])
    .filter(person => person?.role !== 'support')
    .map(person => person?.type)
    .filter(type => ['student', 'guardian', 'staff'].includes(type)))]
  Object.assign(intervention, {
    id: item.id,
    record_updated_at: item.updated_at || '',
    change_reason: '',
    kind: item.kind || 'entrevista',
    ...
    due_at: dateTimeInput(item.due_at),
    status: item.status || 'borrador',
    confidentiality: item.confidentiality || 'restringido',
    participant_types: participantTypes.length ? [...participantTypes] : ['student'],
    participant_staff_ids: [...(structured.participant_staff_ids || [])],
    support_staff_ids: [...(structured.support_staff_ids || [])],
    commitments: [],
  })
  staffSearch.value = ''
  attentionReturn.value = 'case'
  modal.value = 'attention'
}
...
async function addIntervention() {
  ...
  try {
    const caseId = activeCase.value.id
    const payload = { ...intervention, due_at: intervention.due_at || null }
    const interventionId = payload.id
    delete payload.id
    if (interventionId) await api.patch(`/interventions/${interventionId}`, payload)
    else await api.post(`/cases/${caseId}/interventions`, payload)
    success.value = interventionId ? 'Ficha corregida y respaldada en el historial.' : 'Atención registrada en la ficha.'
    resetIntervention()
...
async function exportCasePdf() {
  if (!activeCase.value || exportingCasePdf.value) return
  exportingCasePdf.value = true
  try {
    const response = await api.post(`/cases/${activeCase.value.id}/export`)
    await downloadSocialCaseMaster(response)
    success.value = 'Expediente PDF descargado con la información autorizada.'
  } catch {} finally {
  ...
  <Teleport to="body">
    <div v-if="modal" class="sw-modal" role="dialog" aria-modal="true" @click.self="modal === 'attention' ? cancelAttention() : closeModal()">
      <div class="sw-modal-panel" :class="{ wide: modal === 'case', attention: modal === 'attention' }">
        <header><div><span class="eyebrow">{{ modal === 'create' ? 'Apertura confidencial' : modal === 'case' ? 'Registro histórico del caso' : intervention.id ? 'Corrección trazable' : 'Nueva actuación profesional' }}</span><h2>{{ modal === 'create' ? 'Nuevo caso social' : modal === 'case' ? title : `${intervention.id ? 'Editar ficha' : 'Registrar atención'} · ${activeCase?.code || ''}` }}</h2></div><button class="close-button" aria-label="Cerrar" @click="modal === 'attention' ? cancelAttention() : closeModal()"><i class="bx bx-x"></i></button></header>
        ...
        <div v-else-if="modal === 'case'" class="modal-body case-sheet">
          ...
          <template v-else>
            <section class="case-summary">
              ...
              <div class="summary-actions"><button v-if="api.can('social_work.reports.export')" class="btn btn-sm case-export-button" :disabled="exportingCasePdf" @click="exportCasePdf"><i class="bx bxs-file-pdf"></i> {{ exportingCasePdf ? 'Preparando expediente…' : 'Exportar caso' }}</button><button v-if="canRegisterAttention && !['cerrado','anulado'].includes(activeCase.status)" class="btn btn-sm btn-primary" @click="openAttention(activeCase.id, true)"><i class="bx bx-message-square-add"></i> Registrar atención</button></div>
            </section>
            <div class="history-layout">
              <main>
                ...
                <section class="history-section"><div class="history-heading"><div><span>02</span><h3>Atenciones e intervenciones</h3></div><small>{{ activeCase.interventions?.length || 0 }} registros</small></div>
                  <div class="timeline">
                    <article v-for="item in activeCase.interventions || []" :key="item.id" class="timeline-row">
                      <div class="timeline-icon"><i class="bx bx-check"></i></div>
                      <div>
                        <div class="timeline-title"><strong>{{ label(item.kind) }} · {{ item.objective }}</strong><span>{{ label(item.status) }}</span></div>
                        <small>{{ date(item.activity_date) }} · {{ item.responsible?.name || 'Sin responsable' }} · {{ label(item.confidentiality) }}</small>
                        <div v-if="peopleFor(item, 'participant').length || peopleFor(item, 'support').length" class="timeline-people"><span v-if="peopleFor(item, 'participant').length"><i class="bx bx-group"></i><b>Participantes:</b> {{ peopleFor(item, 'participant').join(', ') }}</span><span v-if="peopleFor(item, 'support').length"><i class="bx bx-user-plus"></i><b>Apoyos:</b> {{ peopleFor(item, 'support').join(', ') }}</span></div>
                        <p>{{ item.result || item.description || 'Sin resumen.' }}</p>
                        <div v-if="item.agreements || item.next_action" class="timeline-followup"><span v-if="item.agreements"><b>Acuerdos:</b> {{ item.agreements }}</span><span v-if="item.next_action"><b>Próxima acción:</b> {{ item.next_action }} · {{ date(item.due_at) }}</span></div>
                        <button v-if="canRegisterAttention && !['cerrado','anulado'].includes(activeCase.status) && item.status !== 'anulada'" type="button" class="timeline-edit" @click="openInterventionEdit(item)"><i class="bx bx-edit-alt"></i> Editar ficha</button>
                      </div>
                    </article>
                    <p v-if="!activeCase.interventions?.length" class="empty">Aún no existen atenciones registradas.</p>
                  </div>
                </section>
              </main>
        ...
        <form v-else class="modal-body attention-form" @submit.prevent="addIntervention">
          ...
          <template v-else>
            ...
            <section class="form-section"><h3><span>4</span> Seguimiento</h3><div class="row g-3"><div class="col-md-8"><label>Próxima acción</label><input v-model="intervention.next_action" class="form-control" placeholder="Acción de seguimiento acordada"></div><div class="col-md-4"><label>Fecha límite</label><input v-model="intervention.due_at" type="datetime-local" class="form-control"></div><div class="col-md-4"><label>Estado del registro</label><select v-model="intervention.status" class="form-select" :disabled="Boolean(intervention.id && intervention.status === 'finalizada')"><option value="finalizada">Finalizada</option><option value="borrador">Borrador</option></select></div></div></section>
            <section v-if="intervention.id" class="form-section correction-section"><h3><span>5</span> Trazabilidad de la corrección</h3><p class="section-help">La versión anterior y la nueva quedarán cifradas en el historial.</p><label>Motivo de la corrección *</label><textarea v-model="intervention.change_reason" class="form-control" rows="2" minlength="5" maxlength="1000" required placeholder="Ej.: Se corrige la fecha y el acuerdo luego de revisar el acta."></textarea></section>
            ...
            <footer><button type="button" class="btn btn-light" @click="cancelAttention">Cancelar</button><button class="btn btn-primary" :disabled="api.loading || !canSubmitAttention"><i class="bx bx-save"></i> {{ intervention.id ? 'Guardar corrección' : 'Guardar atención' }}</button></footer>
          </template>
        </form>
      </div>
    </div>
...
.historical-date-help{display:block;margin-top:.3rem;color:#748297;font-size:.67rem;line-height:1.35}.case-export-button{display:inline-flex;align-items:center;gap:.4rem;color:#6f3d50;background:linear-gradient(135deg,#fff8f8,#f8edf1);border:1px solid #e5c8d2;font-weight:800;box-shadow:0 6px 15px rgba(116,61,80,.08)}.case-export-button:hover,.case-export-button:focus-visible{color:#583043;background:#f7e8ed;border-color:#d7a9b8}.case-export-button i{color:#b64760;font-size:1rem}
.timeline-edit{display:inline-flex;align-items:center;gap:.35rem;margin-top:.65rem;padding:.38rem .65rem;color:#17685c;background:#eef9f6;border:1px solid #bfe4da;border-radius:9px;font-size:.72rem;font-weight:800}.timeline-edit:hover,.timeline-edit:focus-visible{color:#0f5047;border-color:#75c4b0;box-shadow:0 5px 14px rgba(23,104,92,.11)}.correction-section{background:linear-gradient(135deg,#f0f8ff,#f6f1fc);border-color:#d8e1f0}.correction-section .section-help{margin:0 0 .45rem 1.85rem}
</style>

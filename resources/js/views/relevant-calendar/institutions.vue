...
export default {
  ...
  methods: {
    async load() {
      ...
      this.error = null;
      try {
        const response = await axios.get("/api/relevant-calendar/institutions", {
    ...
    async save() {
      ...
      try {
        const payload = {
          ...
          website_url: this.form.website_url || null,
          color: this.form.color || null,
          is_active: this.form.is_active,
        };
        if (this.isEditing) {
          await axios.put(`/api/relevant-calendar/institutions/${this.form.id}`, payload);
        } else {
          await axios.post("/api/relevant-calendar/institutions", payload);
        }
    ...
    async toggle(item) {
      try {
        await axios.put(`/api/relevant-calendar/institutions/${item.id}/active`, {
    ...
    async remove(item) {
      ...
      try {
        await axios.delete(`/api/relevant-calendar/institutions/${item.id}`);
        this.load();
      } catch (error) {
        this.error = this.formatError(error);
      }
...
<template>
  <Layout>
    <div class="d-flex justify-content-between align-items-center mb-3">
      <div>
        <h4 class="mb-0">Instituciones</h4>
        <div class="text-muted">Catálogo editable de organismos y entidades relacionadas.</div>
      </div>
      <div class="d-flex gap-2">
        <router-link to="/relevant-calendar" class="btn btn-outline-secondary">Volver</router-link>
        <BButton variant="primary" @click="openCreate">Nueva institución</BButton>
      </div>
    </div>
    ...
    <BCard class="mb-3">
      <div class="row g-3 align-items-end">
        <div class="col-md-4">
          ...
          <BFormInput v-model="search" placeholder="Nombre, descripción o URL" @keyup.enter="load" />
        </div>
        <div class="col-md-3">
    ...
    <BCard>
      ...
      >
        <template #table-busy>
          <LoadingState message="Cargando instituciones..." compact />
        </template>
        <template #cell(name)="{ item }">
          <div class="d-flex align-items-center gap-2">
            ...
            <div>
              ...
              <div class="text-muted small">{{ item.website_url || item.description || "-" }}</div>
            </div>
          </div>
        </template>
    ...
    <BModal v-model="showModal" :title="isEditing ? 'Editar institución' : 'Nueva institución'" hide-footer>
      <BAlert v-if="error" variant="danger" show class="mb-3">{{ error }}</BAlert>
      <div class="row g-3">
        <div class="col-md-8">
        ...
        <div class="col-md-12">
          ...
          <BFormInput v-model="form.website_url" type="url" placeholder="https://..." />
        </div>
        <div class="col-md-12">
        ...
        <div class="col-md-12">
          <BFormCheckbox v-model="form.is_active">Activa</BFormCheckbox>
        </div>
      </div>

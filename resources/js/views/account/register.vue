  ...
  <div class="account-pages my-5 pt-5">
    <BContainer>
      <BRow class="justify-content-center">
        <BCol md="8" lg="6" xl="5">
          <BCard o-body class="overflow-hidden">
            ...
            <BCardBody class="pt-0">
              ...
              <BForm class="p-2" action="javascript:void(0)" method="POST">
                <slot />
                <BFormGroup id="email-group" label="Name" label-for="name" class="mb-3">
                  <BFormInput id="name" v-model="auth.name" name="name" type="text" placeholder="Enter name"></BFormInput>
                </BFormGroup>
    
                <BFormGroup id="fullname-group" label="Email" label-for="email" class="mb-3">
                  <BFormInput id="email" name="email" v-model="auth.email" type="email" placeholder="Enter email"></BFormInput>
                </BFormGroup>
    
                <BFormGroup id="password-group" label="Password" label-for="password" class="mb-3">
                  <BFormInput id="password" v-model="auth.password" name="password" type="password"
                    placeholder="Enter password"></BFormInput>
                </BFormGroup>
                <BFormGroup label="Confirm Password" label-for="password-confirm" class="mb-3">
                  <BFormInput id="password-confirm" v-model="auth.password_confirmation" name="password_confirmation"
                    type="password" placeholder="Confirm password"></BFormInput>
                </BFormGroup>
    
                <div class="mt-4 d-grid">
                  <BButton type="submit" :disabled="processing" @click="register" variant="primary" class="btn-block">

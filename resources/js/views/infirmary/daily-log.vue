...
<template>
  <Layout>
    <main class="nursing-log-page">
      ...
      <BModal v-model="showFileModal" size="lg" title="Ficha de bitácora" hide-footer>
        <article v-if="selectedEntry" class="log-file">
          <header><div><span>{{ optionLabel(categories, selectedEntry.category) }}</span><h3>{{ selectedEntry.title }}</h3><p>{{ formatDate(selectedEntry.happened_at) }}</p></div><span class="priority-chip" :class="selectedEntry.priority">{{ optionLabel(priorities, selectedEntry.priority) }}</span></header>
          <div class="log-file__meta"><section><span>Alumna</span><strong>{{ selectedEntry.student ? studentName(selectedEntry.student) : "Registro general" }}</strong><small>{{ selectedEntry.course_section?.display_name || "Sin curso asociado" }}</small></section><section><span>Responsable</span><strong>{{ selectedEntry.registered_by?.name || "Enfermería" }}</strong><small>{{ optionLabel(statuses, selectedEntry.status) }}</small></section></div>
          <section class="log-file__body"><span>Descripción del hecho</span><p>{{ selectedEntry.detail }}</p></section>
          <section v-if="selectedEntry.action_taken" class="log-file__body is-action"><span>Acción o medida realizada</span><p>{{ selectedEntry.action_taken }}</p></section>
          <section v-if="selectedEntry.requires_follow_up" class="log-file__follow"><i class="bx bx-time-five"></i><div><strong>Seguimiento requerido</strong><p>{{ selectedEntry.follow_up_note || "Sin indicación adicional." }}</p></div></section>
          <div class="form-actions"><button type="button" class="btn btn-light" @click="showFileModal=false">Cerrar</button><button v-if="capabilities.can_manage" type="button" class="btn nursing-log-submit" @click="showFileModal=false; openEdit(selectedEntry)"><i class="bx bx-edit"></i>Editar registro</button></div>
        </article>
      </BModal>
    </main>
  </Layout>

  ...
  methods: {
    ...
    async destroyItem(item) {
      ...
      const confirmation = await Swal.fire({
        icon: "warning",
        title: `¿Eliminar ${name}?`,
        text: "Esta acción sólo se completará si el registro no tiene existencias ni movimientos históricos.",
      ...
      try {
        ...
        await this.loadAll();
        Swal.fire({ icon: "success", title: "Registro eliminado", text: data.message, timer: 2100, showConfirmButton: false });
      } catch (error) {
        this.alertError(error);
      } finally {
    ...
    selectedItem(id) { return this.items.find((item) => Number(item.id) === Number(id)); },
...
<template>
  <Layout>
    ...
    <BModal v-model="showDeliveryModal" size="xl" hide-footer :title="isMaintenanceStoreroom ? 'Registrar salida y emitir acta' : 'Registrar entrega y emitir acta'" modal-class="supply-modal">
      <form class="supply-form" @submit.prevent="saveDelivery">
        <div class="form-banner form-banner--out"><i class="bx bx-file"></i><div><strong>{{ isMaintenanceStoreroom ? "Salida desde el pañol" : "Acta de entrega" }}</strong><p>Al confirmar se descontará el stock y se descargará el PDF listo para firmar.</p></div></div>
        <div class="form-grid delivery-form-grid">
          <label>Fecha de entrega<input v-model="deliveryForm.delivered_at" type="date" required /></label>
          <label class="wide">Funcionario receptor
            <Multiselect v-model="deliveryForm.recipient_staff_id" class="recipient-select" :options="catalogs.delivery_recipients" value-prop="id" label="name" :searchable="true" :can-clear="true" :close-on-select="true" placeholder="Buscar personal que recibe OT..." no-options-text="No hay personal marcado para recibir OT" no-results-text="No encontramos funcionarios" @change="onDeliveryRecipientChange">
          ...
          <label>Destino / dependencia<input v-model.trim="deliveryForm.destination" maxlength="255" placeholder="Ej.: Auxiliares, internado..." /></label>
          <label class="wide">Observaciones generales<textarea v-model.trim="deliveryForm.notes" rows="2"></textarea></label>
        </div>
        <section class="lines-editor"><header><div><span>{{ isMaintenanceStoreroom ? "HERRAMIENTAS Y ARTÍCULOS A ENTREGAR" : "INSUMOS A ENTREGAR" }}</span><strong>{{ deliveryForm.items.length }} línea(s)</strong></div><button type="button" @click="addDeliveryLine"><i class="bx bx-plus"></i> Agregar {{ itemNoun }}</button></header><div v-for="(line, index) in deliveryForm.items" :key="index" class="editor-line editor-line--delivery"><span class="line-number">{{ index + 1 }}</span><label>{{ isMaintenanceStoreroom ? "Herramienta / artículo" : "Insumo" }}<select v-model="line.supply_item_id" required><option :value="null" disabled>Seleccionar...</option><option v-for="item in activeItems" :key="item.id" :value="item.id" :disabled="Number(item.inventory_item?.stock_quantity || 0) <= 0">{{ item.inventory_item?.name }} · disponible {{ number(item.inventory_item?.stock_quantity) }} {{ unitLabel(item.inventory_item?.unit_of_measure) }}</option></select></label><label>Cantidad<input v-model.number="line.quantity" type="number" min="0.01" :max="selectedItem(line.supply_item_id)?.inventory_item?.stock_quantity" step="0.01" required /></label><label>Observación<input v-model.trim="line.notes" maxlength="255" placeholder="Opcional" /></label><button type="button" class="remove-line" :disabled="deliveryForm.items.length === 1" @click="removeLine(deliveryForm.items, index)"><i class="bx bx-trash"></i></button></div></section>
        <footer><button type="button" class="btn btn-light" @click="showDeliveryModal = false">Cancelar</button><button class="btn btn-primary" :disabled="saving || !deliveryForm.recipient_staff_id"><i class="bx bxs-file-pdf"></i> {{ saving ? 'Registrando...' : (isMaintenanceStoreroom ? 'Registrar salida y descargar acta' : 'Registrar y descargar acta') }}</button></footer>
      </form>
    </BModal>
  </Layout>
</template>

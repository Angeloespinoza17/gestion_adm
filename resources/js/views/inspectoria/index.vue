...
export default {
  ...
  data() {
    return {
      ...
      saving: false,
      error: null,
      catalogs: { academic_years: [], courses: [], students: [], inspectors: [], staff: [], request_types: [], attention_actions: [], psychosocial_professionals: [], destinations: [], log_categories: [], withdrawal_statuses: [], withdrawal_reasons: [], withdrawal_relationships: [], pickup_restriction_types: [], capabilities: {} },
      tabs: [
  ...
  computed: {
    ...
    visibleTabs() { return this.tabs.filter((tab) => tab.key !== "statistics" || this.catalogs.capabilities?.view_statistics); },
    activeMeta() {
      const meta = {
...
<template>
  <Layout>
    <main class="inspectoria-page">
          ...
          <div class="row g-3">
            ...
            <template v-if="isStaffLateness">
              <div class="col-12 staff-lateness-fields">
                ...
                <div><label class="form-label d-block">Cursos asociados <small class="text-muted">(opcional, puedes seleccionar varios)</small></label><div class="staff-lateness-courses"><button v-for="course in catalogs.courses" :key="course.id" type="button" :class="{ selected: dailyLog.associated_course_ids.includes(course.id) }" @click="toggleStaffLatenessCourse(course.id)"><i :class="dailyLog.associated_course_ids.includes(course.id) ? 'bx bx-check-circle' : 'bx bx-circle'"></i>{{ course.display_name }}</button></div></div>
      ...
      <BModal v-model="showStudentFile" size="xl" title="Ficha de alumna · Inspectoría" scrollable hide-footer>
        ...
        <div v-else-if="selectedStudentFile" class="student-file student-file--redesigned">
          ...
          <section class="student-file-contact-section">
            <div class="student-file-section-title"><div><span>RED DE CONTACTO</span><h4>Personas responsables y alertas</h4></div><small>Información disponible para gestión inmediata</small></div>
...
@media(max-width:1100px){.student-file-summary{grid-template-columns:repeat(2,minmax(0,1fr))}.student-file-contacts{grid-template-columns:repeat(2,minmax(0,1fr))}.student-alert-card{grid-column:1/-1}.student-file-history-grid{grid-template-columns:1fr}.student-history-list{max-height:290px;min-height:0}}
@media(max-width:768px){.student-file>.student-file-hero{align-items:flex-start;flex-wrap:wrap;padding:1rem}.student-file-avatar{flex-basis:52px;height:52px}.student-file-identity{max-width:calc(100% - 68px)}.student-file .student-file-identity h3{font-size:1.05rem;white-space:normal}.student-file-status{margin-left:0}.student-file-summary,.student-file-contacts{grid-template-columns:1fr}.student-alert-card{grid-column:auto}.student-file-section-title{align-items:flex-start;flex-direction:column;gap:.2rem}.student-file-meta{align-items:flex-start;flex-direction:column}.student-history-heading{align-items:flex-start;flex-wrap:wrap}.student-history-heading time{width:100%}}
</style>

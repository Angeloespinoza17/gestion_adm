...
class CreateReportRequest extends FormRequest
{
    ...
    {
        $curriculum = $this->input('report_type') === 'curriculum_objectives';
        $filterRules = ['sometimes', 'array'];
        if ($curriculum) {
            $filterRules[] = 'array:level_code,grade_code,curriculum_track,subject_code,subject,schedule_subject_id,objective_type,axis_code,status,source,query,catalog_id,course_section_id';
        }
        ...
        return [
            ...
            'report_type' => ['required', Rule::in([
                ...
                'amendment_history', 'audit', 'closure_status', 'executive', 'curriculum_objectives',
            ])],
            'format' => ['required', Rule::in($curriculum ? ['pdf', 'xlsx'] : ['pdf', 'xlsx', 'csv', 'json'])],
            'filters' => $filterRules,
